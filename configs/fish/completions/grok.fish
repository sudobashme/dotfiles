...
complete -c grok -n "__fish_grok_needs_command" -l compaction-mode -d 'Compaction mode [summary|transcript|segments]. `summary` adds no pointer; `transcript` points at the raw transcript; `segments` (default) persists per-segment markdown to grep. Sets `GROK_COMPACTION_MODE`' -r
...
complete -c grok -n "__fish_grok_needs_command" -s r -l resume -d 'Resume a session by ID or title, or the most recent if omitted. Non-ID values match session titles for the current directory, ignoring letter case; UUID-shaped values always mean IDs. Among duplicate titles a sole renamed match wins; otherwise the resume fails as ambiguous' -r
...
complete -c grok -n "__fish_grok_needs_command" -s w -l worktree -d 'Start the session in a new git worktree, optionally named. With `--resume` of a remote session, pass `--restore-code` to apply the snapshot codebase (conversation is restored either way)' -r
...
complete -c grok -n "__fish_grok_needs_command" -l todo-gate -d 'Enable the runtime turn-end TodoGate for this session. Session-scoped (not persisted). Highest precedence: overrides remote `todo_gate_enabled` and the built-in default (which is `false`)'
complete -c grok -n "__fish_grok_needs_command" -l no-alt-screen -d 'Run inline instead of using the terminal alternate screen'
complete -c grok -n "__fish_grok_needs_command" -l minimal -d 'Experimental: scrollback-native rendering. Finalized blocks are printed into the terminal\'s native scrollback (use the terminal\'s own scroll / selection). Session-scoped only, does not write config'
...
complete -c grok -n "__fish_grok_needs_command" -a "usage" -d 'Print persisted token and cost usage for a session'
...
complete -c grok -n "__fish_grok_needs_command" -a "wrap" -d 'Run any command with local clipboard support (forwards OSC 52 to the system clipboard)'
...
complete -c grok -n "__fish_grok_needs_command" -a "clone" -d 'Lazy-clone a git repository via grove (NFS on macOS, FUSE on Linux)'
...
complete -c grok -n "__fish_grok_needs_command" -a "cursor-worker" -d 'Register this machine as a Cursor private worker (via the leader)'
complete -c grok -n "__fish_grok_needs_command" -a "dashboard" -d 'Open the Agent Dashboard view at startup. The dashboard shows every session, top-level and subagents. Disabled when `[dashboard].enabled = false` in `~/.grok/config.toml` or when the `GROK_AGENT_DASHBOARD=0` env var is set'
...
complete -c grok -n "__fish_grok_using_subcommand agent; and not __fish_seen_subcommand_from stdio headless serve leader help" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand agent; and __fish_seen_subcommand_from stdio" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand agent; and __fish_seen_subcommand_from headless" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand agent; and __fish_seen_subcommand_from leader" -l cursor-worker-name -d 'Worker display name shown in Cursor (default: `[cursor_worker].name`, then the hostname)' -r
complete -c grok -n "__fish_grok_using_subcommand agent; and __fish_seen_subcommand_from leader" -l cursor-worker-dir -d 'Repository directory offered to Cursor Cloud Agents; repeatable, the first one is the registered working directory. Defaults to `[cursor_worker].worker_dirs`' -r -f -a "(__fish_complete_directories)"
complete -c grok -n "__fish_grok_using_subcommand agent; and __fish_seen_subcommand_from leader" -l cursor-worker-max-agents -d 'Maximum concurrent Cloud Agent claims (default: `[cursor_worker].max_agents`; unset means no cap)' -r
...
complete -c grok -n "__fish_grok_using_subcommand agent; and __fish_seen_subcommand_from leader" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand agent; and __fish_seen_subcommand_from leader" -l relay-on-demand -d 'Defer the grok.com relay WebSocket until the first headless IPC client registers. Without this flag the leader connects the relay eagerly at startup. Passed by leaders auto-spawned from interactive clients (TUI/IDE), which only need the relay if a headless client appears'
...
complete -c grok -n "__fish_grok_using_subcommand agent; and __fish_seen_subcommand_from leader" -l cursor-worker -d 'Register this leader as a Cursor private worker once it is ready. Without this flag the door starts only when `[cursor_worker] auto_start = true`'
...
complete -c grok -n "__fish_grok_using_subcommand inspect" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand doctor; and not __fish_seen_subcommand_from fix help" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand doctor; and __fish_seen_subcommand_from fix" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand leader; and not __fish_seen_subcommand_from list info kill help" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand leader; and __fish_seen_subcommand_from info" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand leader; and __fish_seen_subcommand_from kill" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand logout" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand login" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand login" -l devbox -d 'Authenticate for remote development environments (hidden). Field is always present so match arms stay feature-unification-safe; clap registers `--devbox` only when that feature is enabled (`arg(skip)` otherwise → always false)'
...
complete -c grok -n "__fish_grok_using_subcommand mcp; and not __fish_seen_subcommand_from list add remove enable disable doctor help" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand mcp; and __fish_seen_subcommand_from list" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand mcp; and __fish_seen_subcommand_from add" -s t -l transport -d 'Transport type. Defaults to stdio, or to http when the positional argument is an http(s):// URL' -r -f -a "stdio\t'Launch a local process and communicate over stdin/stdout'
...
complete -c grok -n "__fish_grok_using_subcommand mcp; and __fish_seen_subcommand_from add" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
project\t'`./.grok/config.toml`, shared with everyone working in this directory'"
complete -c grok -n "__fish_grok_using_subcommand mcp; and __fish_seen_subcommand_from remove" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand mcp; and __fish_seen_subcommand_from enable" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand mcp; and __fish_seen_subcommand_from disable" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand mcp; and __fish_seen_subcommand_from doctor" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand plugin; and not __fish_seen_subcommand_from list install uninstall rm remove update enable disable details validate tag marketplace help" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand plugin; and __fish_seen_subcommand_from list" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand plugin; and __fish_seen_subcommand_from install" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand plugin; and __fish_seen_subcommand_from uninstall" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand plugin; and __fish_seen_subcommand_from update" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand plugin; and __fish_seen_subcommand_from validate" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand plugin; and __fish_seen_subcommand_from tag" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand plugin; and __fish_seen_subcommand_from marketplace" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand sessions; and not __fish_seen_subcommand_from list search delete help" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand sessions; and __fish_seen_subcommand_from list" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand sessions; and __fish_seen_subcommand_from search" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand sessions; and __fish_seen_subcommand_from delete" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand usage" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
complete -c grok -n "__fish_grok_using_subcommand usage" -l debug-file -d 'Write debug logs to FILE' -r -F
complete -c grok -n "__fish_grok_using_subcommand usage" -l debug -d 'Enable debug logging'
complete -c grok -n "__fish_grok_using_subcommand usage" -s h -l help -d 'Print help'
complete -c grok -n "__fish_grok_using_subcommand setup" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand share" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand wrap" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand export" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand trace" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand version" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand v" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand completions" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from ls" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from show" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from gc" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from detach" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from detach" -l debug-file -d 'Write debug logs to FILE' -r -F
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from detach" -l allow-copy -d 'Permit a copy when dest and backing are on different devices'
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from detach" -l debug -d 'Enable debug logging'
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from detach" -s h -l help -d 'Print help'
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from salvage" -l out -d 'Directory to write the salvaged plain repo into' -r
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from salvage" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from salvage" -l debug-file -d 'Write debug logs to FILE' -r -F
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from salvage" -l debug -d 'Enable debug logging'
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from salvage" -s h -l help -d 'Print help'
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from clean-artifacts" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from clean-artifacts" -l debug-file -d 'Write debug logs to FILE' -r -F
complete -c grok -n "__fish_grok_using_subcommand worktree; and __fish_seen_subcommand_from clean-artifacts" -l dry-run -d 'List jail children and exit without deleting'
...
complete -c grok -n "__fish_grok_using_subcommand workspace; and not __fish_seen_subcommand_from start pause resume stop restart status list help" -f -a "start" -d 'Start (or update) the workspace-to-hub exposure'
...
complete -c grok -n "__fish_grok_using_subcommand workspace; and __fish_seen_subcommand_from start" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand workspace; and __fish_seen_subcommand_from pause" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand workspace; and __fish_seen_subcommand_from stop" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand workspace; and __fish_seen_subcommand_from restart" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand workspace; and __fish_seen_subcommand_from status" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand workspace; and __fish_seen_subcommand_from help" -f -a "start" -d 'Start (or update) the workspace-to-hub exposure'
...
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and not __fish_seen_subcommand_from start stop status list help" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and not __fish_seen_subcommand_from start stop status list help" -l debug-file -d 'Write debug logs to FILE' -r -F
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and not __fish_seen_subcommand_from start stop status list help" -l debug -d 'Enable debug logging'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and not __fish_seen_subcommand_from start stop status list help" -s h -l help -d 'Print help'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and not __fish_seen_subcommand_from start stop status list help" -f -a "start" -d 'Register the leader as a Cursor private worker (or restart it with new options)'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and not __fish_seen_subcommand_from start stop status list help" -f -a "stop" -d 'Unregister from Cursor and release the claims (the leader keeps running)'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and not __fish_seen_subcommand_from start stop status list help" -f -a "status" -d 'Show the current worker status and its Cloud Agent claims'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and not __fish_seen_subcommand_from start stop status list help" -f -a "list" -d 'Show the current worker status and its Cloud Agent claims'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and not __fish_seen_subcommand_from start stop status list help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from start" -l name -d 'Worker display name shown in Cursor (default: `[cursor_worker].name`, then the hostname)' -r
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from start" -l worker-dir -d 'Repository directory offered to Cursor Cloud Agents; repeatable, the first one is the registered working directory. Defaults to `[cursor_worker].worker_dirs`' -r -f -a "(__fish_complete_directories)"
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from start" -l max-agents -d 'Maximum concurrent Cloud Agent claims (default: `[cursor_worker].max_agents`; unset means no cap)' -r
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from start" -l timeout -d 'Seconds to wait for the registration before printing the status; `0` returns as soon as the leader accepted the start' -r
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from start" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from start" -l debug-file -d 'Write debug logs to FILE' -r -F
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from start" -l leader -d 'Force leader mode for this command, overriding config'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from start" -l no-leader -d 'Refuse to start even when config enables leader mode'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from start" -l json -d 'Emit machine-readable JSON output'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from start" -l debug -d 'Enable debug logging'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from start" -s h -l help -d 'Print help'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from stop" -l pid -d 'Leader process ID from `grok leader list`' -r
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from stop" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from stop" -l debug-file -d 'Write debug logs to FILE' -r -F
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from stop" -l json -d 'Emit machine-readable JSON output'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from stop" -l debug -d 'Enable debug logging'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from stop" -s h -l help -d 'Print help'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from status" -l pid -d 'Leader process ID from `grok leader list`' -r
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from status" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from status" -l debug-file -d 'Write debug logs to FILE' -r -F
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from status" -l json -d 'Emit machine-readable JSON output'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from status" -l debug -d 'Enable debug logging'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from status" -s h -l help -d 'Print help'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from list" -l pid -d 'Leader process ID from `grok leader list`' -r
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from list" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from list" -l debug-file -d 'Write debug logs to FILE' -r -F
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from list" -l json -d 'Emit machine-readable JSON output'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from list" -l debug -d 'Enable debug logging'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from list" -s h -l help -d 'Print help'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from help" -f -a "start" -d 'Register the leader as a Cursor private worker (or restart it with new options)'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from help" -f -a "stop" -d 'Unregister from Cursor and release the claims (the leader keeps running)'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from help" -f -a "status" -d 'Show the current worker status and its Cloud Agent claims'
complete -c grok -n "__fish_grok_using_subcommand cursor-worker; and __fish_seen_subcommand_from help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
complete -c grok -n "__fish_grok_using_subcommand dashboard" -l leader-socket -d 'Use a custom leader socket path instead of the default `~/.grok/leader.sock`. A local/branch build can thus run an isolated leader without colliding with the default one already running on the machine Name it `~/.grok/leader-*.sock` to keep `grok leader list/kill` able to find it; any other location works but won\'t be auto-discovered' -r -F
...
complete -c grok -n "__fish_grok_using_subcommand dashboard" -s h -l help -d 'Print help'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "agent" -d 'Run Grok without the interactive UI'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "inspect" -d 'Show the configuration Grok discovers for this directory'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "doctor" -d 'Check terminal, clipboard, color, and input support without starting Grok'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "leader" -d 'Manage running leader processes'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "logout" -d 'Sign out and clear cached credentials'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "login" -d 'Sign in to Grok'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "mcp" -d 'Manage MCP server configurations'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "plugin" -d 'Manage plugins and marketplace sources'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "memory" -d 'Manage cross-session memory'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "models" -d 'List available models and exit'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "sessions" -d 'List, search, or restore sessions'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "usage" -d 'Print persisted token and cost usage for a session'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "setup" -d 'Fetch and install managed configuration'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "share" -d 'Share a session and print the share URL'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "wrap" -d 'Run any command with local clipboard support (forwards OSC 52 to the system clipboard)'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "export" -d 'Export a session transcript as Markdown'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "trace" -d 'Export or upload session trace data'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "update" -d 'Check for updates or install a specific version'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "version" -d 'Print version information'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "completions" -d 'Generate shell completion scripts (bash, zsh, fish, powershell, ...)'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "worktree" -d 'Manage git worktrees'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "clone" -d 'Lazy-clone a git repository via grove (NFS on macOS, FUSE on Linux)'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "du" -d 'Show what the grok home (~/.grok) uses on disk'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "workspace" -d 'Expose this workspace to the Computer Hub (via the leader)'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "cursor-worker" -d 'Register this machine as a Cursor private worker (via the leader)'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "dashboard" -d 'Open the Agent Dashboard view at startup. The dashboard shows every session, top-level and subagents. Disabled when `[dashboard].enabled = false` in `~/.grok/config.toml` or when the `GROK_AGENT_DASHBOARD=0` env var is set'
complete -c grok -n "__fish_grok_using_subcommand help; and not __fish_seen_subcommand_from agent inspect doctor leader logout login mcp plugin memory models sessions usage setup share wrap export trace update version completions worktree clone du workspace cursor-worker dashboard help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
...
complete -c grok -n "__fish_grok_using_subcommand help; and __fish_seen_subcommand_from cursor-worker" -f -a "start" -d 'Register the leader as a Cursor private worker (or restart it with new options)'
complete -c grok -n "__fish_grok_using_subcommand help; and __fish_seen_subcommand_from cursor-worker" -f -a "stop" -d 'Unregister from Cursor and release the claims (the leader keeps running)'
complete -c grok -n "__fish_grok_using_subcommand help; and __fish_seen_subcommand_from cursor-worker" -f -a "status" -d 'Show the current worker status and its Cloud Agent claims'
