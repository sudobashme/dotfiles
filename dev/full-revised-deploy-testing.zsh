...
detect_os() {
  ...
  *)
  ...
  echo "${FMT_BLUE} Detected OS: ${FMT_RESET} ${FMT_GREEN} $OS ${FMT_RESET}"
  if [[ $OS == "Linux" ]]; then
    echo "${FMT_BLUE} Detected Distro: ${FMT_RESET} ${FMT_GREEN} $DISTRO ${FMT_RESET}"
  fi
}
...
set_dot_files_repo_location() {
  ...
  if [[ ! -d $DOTFILES ]]; then
    if ! command -v git &>/dev/null; then
      echo ""
      echo "${FMT_RED} git was not detected and is being installed now... ${FMT_RESET}"
      if [[ $OS == "macOS" ]]; then
...
install_package_manager() {
  if [[ $OS == "macOS" ]]; then
    ...
    if ! command -v yay &>/dev/null; then
      echo "${FMT_RED} yay (AUR helper) not detected, installing now... ${FMT_RESET}"
...
install_fonts() {
  ...
  if [[ $OS == "macOS" ]]; then
    ...
    echo "${FMT_BLUE} installing nerd fonts on Arch... ${FMT_RESET}"
    sudo pacman -Syu --noconfirm ttf-nerd-fonts-symbols ttf-nerd-fonts-symbols-mono
    yay -Syu --noconfirm ttf-hack-nerd ttf-iosevka-nerd ttf-victor-mono-nerd ttf-ubuntu-nerd ttf-mononoki-nerd ttf-jetbrains-mono-nerd
    # Add more as needed, equivalents to the brew ones
    fc-cache -fv
  fi
}

# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
# install apps - adapted for OS
install_apps_description="install common apps (adapted from brewfile for macOS, equivalents for Arch)"
install_apps() {
  if [[ $OS == "macOS" ]]; then
    ...
    # Assuming brewfile has apps like git, tmux, nvim, etc. - map to pacman/yay
    # You may need to create a separate list or parse brewfile, but for simplicity:
    sudo pacman -Syu --noconfirm git tmux neovim zsh fzf ripgrep fd bat eza tree htop btop curl wget unzip zip python python-pip rustup nodejs npm go lua lua-language-server ruby
    yay -Syu --noconfirm visual-studio-code-bin spotify google-chrome # Examples, adjust based on your brewfile
    # For Hyprland specifically
    if [[ $DISTRO == "Arch" ]]; then
      sudo pacman -Syu --noconfirm hyprland waybar wofi dunst polkit-gnome xdg-desktop-portal-hyprland qt5-wayland qt6-wayland
      # Link Hyprland configs if in dotfiles
      if [[ -d "${DOTFILES}/configs/hypr" ]]; then
        zf_ln -sfn "${DOTFILES}/configs/hypr" "${XDG_CONFIG_HOME}/hypr"
      fi
    fi
  fi
}
...
install_zsh_environment() {
  ...
  if command -v nvim &>/dev/null; then
    nvim --headless -c "helptags ALL" -c "qall" &>/dev/null
    nvim --headless -c "TSUpdate" -c "qall" &>/dev/null
    nvim --headless -c "MasonUpdate" -c "qall" &>/dev/null
  fi
...
install_luajit() {
  if [[ $OS == "macOS" ]]; then
    ...
    sudo pacman -Syu --noconfirm make gcc
  fi
...
install_neovim() {
  if ! command -v luajit &>/dev/null; then
    print "must install luajit"
    exit 1
  fi
  if ! command -v lua &>/dev/null; then
    print "must install lua"
    exit 1
  fi
  if [[ $OS == "Linux" && $DISTRO == "Arch" ]]; then
    sudo pacman -Syu --noconfirm base-devel cmake unzip ninja tree-sitter curl gettext
  elif [[ $OS == "macOS" ]]; then
    brew install cmake ninja libtool automake pkg-config gettext curl
  fi
  ...
  if command -v pyenv &>/dev/null; then
    ...
    pip install pip pynvim
  fi
  if command -v gem &>/dev/null; then gem install neovim; fi
...
__install_goenv() {
  ...
  zf_mkdir -p "${GOENV_ROOT}"/{plugins,zsh,man/man1}
...
__install_luaenv() {
  ...
  zf_mkdir -p "${LUAENV_ROOT}"/{plugins,zsh,man/man1}
...
__install_nodenv() {
  ...
  zf_mkdir -p "${NODENV_ROOT}"/{plugins,zsh,man/man1}
...
__install_pyenv() {
  ...
  zf_mkdir -p "${PYENV_ROOT}"/{plugins,zsh,man/man1}
  git clone https://github.com/pyenv/pyenv-default-packages.git "${PYENV_PLUGINS}/pyenv-default-packages"
...
__install_rbenv() {
  ...
  zf_mkdir -p "${RBENV_ROOT}"/{plugins,zsh,man/man1}
...
deploy_menu() {
  ...
  echo "${FMT_GREEN} install_apps ${FMT_RESET} ${FMT_BLUE} - description $install_apps_description $(fmt_link https://github.com/sudobashme/dotfiles/blob/main/configs/homebrew/homebrew_bundle_file) ${FMT_RESET}"
...
main() {
  ...
  echo "${FMT_BLUE} assumed that you would have homebrew installed on macOS or pacman on Arch, and if you continue ${FMT_RESET}"
  echo "${FMT_BLUE} it will be installed along with git if needed. ${FMT_RESET}"
  echo ""
  ...
  if [[ ${CHOICE} = 'y' ]]; then
    ...
    echo "${FMT_BLUE} Execute the deploy_menu function to see a list of these functions ${FMT_RESET}"
    echo ""
  else
